...
- July 4, 2025. Implemented role switching functionality enabling users to change roles (student/mentor) with same email address: added role intent caching system to preserve role switching during OpenID Connect authentication flow, enhanced upsert logic to prioritize role intent over existing roles, added comprehensive logging for authentication debugging, fixed TypeScript errors with proper User type definitions in useAuth hook
